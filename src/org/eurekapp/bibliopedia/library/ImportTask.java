...
package org.eurekapp.bibliopedia.library;
...
public class ImportTask extends QueueableAsyncTask<File, Integer, Void> implements OnCancelListener {
    ...
    private void doInBackground(File parent) {
        ...
        if ( ! parent.exists() ) {
            ...
            importFailed = String.format( context.getString(org.eurekapp.bibliopedia.R.string.no_such_folder), parent.getPath());
            return;
        }
    ...
    public void doOnProgressUpdate(Integer... values) {
		...
		if ( values[0] == UPDATE_IMPORT ) {
			message = String.format(context.getString(org.eurekapp.bibliopedia.R.string.importing), values[1], values[2]);
		} else {
			message = String.format(context.getString(org.eurekapp.bibliopedia.R.string.scan_folders), values[1]);
		}
