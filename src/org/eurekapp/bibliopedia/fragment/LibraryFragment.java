...
import org.eurekapp.bibliopedia.library.ImportTask;
import org.eurekapp.bibliopedia.library.KeyedQueryResult;
import org.eurekapp.bibliopedia.library.KeyedResultAdapter;
import org.eurekapp.bibliopedia.library.LibraryBook;
import org.eurekapp.bibliopedia.library.LibraryService;
import org.eurekapp.bibliopedia.library.QueryResult;
...
import org.eurekapp.bibliopedia.view.BookCaseView;
...
public class LibraryFragment extends RoboSherlockFragment implements ImportCallback {
	...
	@InjectView(org.eurekapp.bibliopedia.R.id.libraryList)
	private ListView listView;
	
	@InjectView(org.eurekapp.bibliopedia.R.id.bookCaseView)
	private BookCaseView bookCaseView;
		
	@InjectView(org.eurekapp.bibliopedia.R.id.alphabetList)
	private ListView alphabetBar;
	
	private AlphabetAdapter alphabetAdapter;
	
	@InjectView(org.eurekapp.bibliopedia.R.id.alphabetDivider)
	private ImageView alphabetDivider;
	
	@InjectView(org.eurekapp.bibliopedia.R.id.libHolder)
	...
	public void onCreate(Bundle savedInstanceState) {			
		...
		Bitmap backupBitmap = BitmapFactory.decodeResource(getResources(), org.eurekapp.bibliopedia.R.drawable.unknown_cover1 );
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		return inflater.inflate(org.eurekapp.bibliopedia.R.layout.fragment_library, container, false);
	}
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		importDialog.setTitle(org.eurekapp.bibliopedia.R.string.importing_books);
		importDialog.setMessage(getString(org.eurekapp.bibliopedia.R.string.scanning_epub));
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<>(actionBar.getThemedContext(),
				...
				android.R.id.text1, getResources().getStringArray(org.eurekapp.bibliopedia.R.array.libraryQueries));
		...
		libraryFolder.match( folder -> {
			...
			Toast.makeText( context, org.eurekapp.bibliopedia.R.string.library_failed, Toast.LENGTH_LONG ).show();
		});
	...
	private void showBookDetails( final LibraryBook libraryBook ) {
		...
		builder.setTitle(org.eurekapp.bibliopedia.R.string.book_details);
		LayoutInflater inflater = PlatformUtil.getLayoutInflater(getActivity());
		
		View layout = inflater.inflate(org.eurekapp.bibliopedia.R.layout.book_details, null);
		builder.setView( layout );
		
		ImageView coverView = (ImageView) layout.findViewById(org.eurekapp.bibliopedia.R.id.coverImage );

		if ( libraryBook.getCoverImage() != null ) {
            Drawable coverDrawable = getCover(libraryBook).getOrElse(
                    getResources().getDrawable(org.eurekapp.bibliopedia.R.drawable.unknown_cover) );

            coverView.setImageDrawable(coverDrawable);
        }

		TextView titleView = (TextView) layout.findViewById(org.eurekapp.bibliopedia.R.id.titleField);
		TextView authorView = (TextView) layout.findViewById(org.eurekapp.bibliopedia.R.id.authorField);
		TextView lastRead = (TextView) layout.findViewById(org.eurekapp.bibliopedia.R.id.lastRead);
		TextView added = (TextView) layout.findViewById(org.eurekapp.bibliopedia.R.id.addedToLibrary);
		TextView descriptionView = (TextView) layout.findViewById(org.eurekapp.bibliopedia.R.id.bookDescription);
		TextView fileName = (TextView) layout.findViewById(org.eurekapp.bibliopedia.R.id.fileName);
		
		titleView.setText(libraryBook.getTitle());
		String authorText = String.format( getString(org.eurekapp.bibliopedia.R.string.book_by),
		...
		if (libraryBook.getLastRead() != null && ! libraryBook.getLastRead().equals(new Date(0))) {
			String lastReadText = String.format(getString(org.eurekapp.bibliopedia.R.string.last_read),
					DATE_FORMAT.format(libraryBook.getLastRead()));
			lastRead.setText( lastReadText );
		} else {
			String lastReadText = String.format(getString(org.eurekapp.bibliopedia.R.string.last_read), getString(org.eurekapp.bibliopedia.R.string.never_read));
			lastRead.setText( lastReadText );
		}

		String addedText = String.format( getString(org.eurekapp.bibliopedia.R.string.added_to_lib),
        ...
        builder.setNeutralButton(org.eurekapp.bibliopedia.R.string.delete, (dialog, which) -> {
		...
		builder.setPositiveButton(org.eurekapp.bibliopedia.R.string.read, (dialog, which) -> openBook(libraryBook) );
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {		
        inflater.inflate(org.eurekapp.bibliopedia.R.menu.library_menu, menu);
        ...
        UiUtils.onMenuPress(menu, org.eurekapp.bibliopedia.R.id.shelves_view).thenDo( toggleListener );
        UiUtils.onMenuPress(menu, org.eurekapp.bibliopedia.R.id.list_view).thenDo( toggleListener );

        UiUtils.onMenuPress(menu, org.eurekapp.bibliopedia.R.id.scan_books).thenDo( this::showImportDialog );
//        onMenuPress( menu, R.id.about ).thenDo( dialogFactory.buildAboutDialog()::show );

        UiUtils.onMenuPress(menu, org.eurekapp.bibliopedia.R.id.profile_day).thenDo(() -> switchToColourProfile(Configuration.ColourProfile.DAY) );
        UiUtils.onMenuPress(menu, org.eurekapp.bibliopedia.R.id.profile_night).thenDo(() -> switchToColourProfile(Configuration.ColourProfile.NIGHT));

        this.searchMenuItem = menu.findItem(org.eurekapp.bibliopedia.R.id.menu_search);
        ...
        if (searchMenuItem != null) {
            ...
            if (searchView != null) {
                ...
                searchMenuItem.setOnMenuItemClickListener( item -> {
                    dialogFactory.showSearchDialog(org.eurekapp.bibliopedia.R.string.search_library, org.eurekapp.bibliopedia.R.string.enter_query, this::performSearch);
                    return false;
                });
            }
        }
...
//        }
    ...
    private void launchFileManager() {
        ...
        try {
            ...
            Toast.makeText(getActivity(), getString(org.eurekapp.bibliopedia.R.string.install_oi),
	...
	public void onPrepareOptionsMenu(Menu menu) {
		...
		menu.findItem(org.eurekapp.bibliopedia.R.id.shelves_view).setVisible(! bookCaseActive);
		menu.findItem(org.eurekapp.bibliopedia.R.id.list_view).setVisible(bookCaseActive);
		menu.findItem(org.eurekapp.bibliopedia.R.id.profile_day).setVisible(config.getColourProfile() == Configuration.ColourProfile.NIGHT);
		menu.findItem(org.eurekapp.bibliopedia.R.id.profile_night).setVisible(config.getColourProfile() == Configuration.ColourProfile.DAY);
	}
	...
	private void showImportDialog() {
		...
		LayoutInflater inflater = PlatformUtil.getLayoutInflater(getActivity());
		final View layout = inflater.inflate(org.eurekapp.bibliopedia.R.layout.import_dialog, null);
		final RadioButton scanSpecific = (RadioButton) layout.findViewById(org.eurekapp.bibliopedia.R.id.radioScanFolder);
		final RadioGroup scanRadioGroup = (RadioGroup) layout.findViewById(org.eurekapp.bibliopedia.R.id.radioScanGroup);
		final TextView folder = (TextView) layout.findViewById(org.eurekapp.bibliopedia.R.id.folderToScan);
		final CheckBox copyToLibrary = (CheckBox) layout.findViewById(org.eurekapp.bibliopedia.R.id.copyToLib);
		final Button browseButton = (Button) layout.findViewById(org.eurekapp.bibliopedia.R.id.browseButton);
		...
		builder.setTitle(org.eurekapp.bibliopedia.R.string.import_books);
    ...
    private void afterImport(int booksImported, List<String> errors, boolean emptyLibrary, boolean silent,
        ...
        if ( (! errors.isEmpty()) ) {
            AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
            builder.setTitle(org.eurekapp.bibliopedia.R.string.import_errors);
        ...
        if ( booksImported > 0 ) {
            ...
            AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
            builder.setTitle(org.eurekapp.bibliopedia.R.string.no_books_found);

            if ( emptyLibrary ) {

                builder.setMessage( getString(org.eurekapp.bibliopedia.R.string.no_bks_fnd_text2) );
                ...
                builder.setMessage( getString(org.eurekapp.bibliopedia.R.string.no_new_books_found));
                builder.setNeutralButton(android.R.string.ok, ( dialog, which) -> dialog.dismiss() );
            }
	...
	public void importFailed(String reason, boolean silent) {
		...
		importDialog.hide();
		AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
		builder.setTitle(org.eurekapp.bibliopedia.R.string.import_failed);
	...
	private class BookListAdapter extends KeyedResultAdapter {	
		...
		public View getView(int index, final LibraryBook book, View convertView,
			...
			if ( convertView == null ) {			
				...
				rowView = inflater.inflate(org.eurekapp.bibliopedia.R.layout.book_row, parent, false);
			} else {
				rowView = convertView;
			}			
			
			TextView titleView = (TextView) rowView.findViewById(org.eurekapp.bibliopedia.R.id.bookTitle);
			TextView authorView = (TextView) rowView.findViewById(org.eurekapp.bibliopedia.R.id.bookAuthor);
			TextView dateView = (TextView) rowView.findViewById(org.eurekapp.bibliopedia.R.id.addedToLibrary);
			TextView progressView = (TextView) rowView.findViewById(org.eurekapp.bibliopedia.R.id.readingProgress);
			
			final ImageView imageView = (ImageView) rowView.findViewById(org.eurekapp.bibliopedia.R.id.bookCover);
			...
			String dateText = String.format(getString(org.eurekapp.bibliopedia.R.string.added_to_lib),
	...
	private class CoverScrollListener implements AbsListView.OnScrollListener {
        ...
        public  CoverScrollListener() {
            try {
                this.holoDrawable = getResources().getDrawable(org.eurekapp.bibliopedia.R.drawable.list_activated_holo);
	...
	private class BookCaseAdapter extends KeyedResultAdapter {
		...
		public View getView(final int index, final LibraryBook object, View convertView,
			...
			if ( convertView == null ) {				
				LayoutInflater inflater = PlatformUtil.getLayoutInflater(getActivity());
				result = inflater.inflate(org.eurekapp.bibliopedia.R.layout.bookcase_row, parent, false);
			...
			final ImageView image = (ImageView) result.findViewById(org.eurekapp.bibliopedia.R.id.bookCover);
			image.setImageDrawable(backupCover);
			TextView text = (TextView) result.findViewById(org.eurekapp.bibliopedia.R.id.bookLabel);
			text.setText( object.getTitle() );
			text.setBackgroundResource(org.eurekapp.bibliopedia.R.drawable.alphabet_bar_bg_dark);
	...
	private void buildImportQuestionDialog() {
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
		builder.setTitle(org.eurekapp.bibliopedia.R.string.no_books_found);
		builder.setMessage( getString(org.eurekapp.bibliopedia.R.string.scan_bks_question) );
	...
	private class AlphabetAdapter extends ArrayAdapter<Character> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if ( tag.equals(highlightChar) ) {
				view.setBackgroundDrawable( getResources().getDrawable(org.eurekapp.bibliopedia.R.drawable.list_activated_holo));
			} else {
    ...
    private void loadQueryData( QueryResult<LibraryBook> result ) {
        ...
        if ( result instanceof KeyedQueryResult && result.getSize() >= ALPHABET_THRESHOLD ) {
            ...
            alphabetAdapter = new AlphabetAdapter(getActivity(),
                    org.eurekapp.bibliopedia.R.layout.alphabet_line, org.eurekapp.bibliopedia.R.id.alphabetLabel,	keyedResult.getAlphabet() );
	...
	private class LoadBooksTask extends QueueableAsyncTask<String, Integer, QueryResult<LibraryBook>> {
        ...
        public void doOnPostExecute(Option<QueryResult<LibraryBook>> result) {
            ...
            }, () -> Toast.makeText(context, org.eurekapp.bibliopedia.R.string.library_failed, Toast.LENGTH_SHORT).show());
