...
package net.eurekapp.pageturner.library;
...
public class ImportTask extends QueueableAsyncTask<File, Integer, Void> implements OnCancelListener {
    ...
    private void doInBackground(File parent) {
        ...
        if ( ! parent.exists() ) {
            ...
            importFailed = String.format( context.getString(net.eurekapp.pageturner.R.string.no_such_folder), parent.getPath());
            return;
        }
    ...
    public void doOnProgressUpdate(Integer... values) {
		...
		if ( values[0] == UPDATE_IMPORT ) {
			message = String.format(context.getString(net.eurekapp.pageturner.R.string.importing), values[1], values[2]);
		} else {
			message = String.format(context.getString(net.eurekapp.pageturner.R.string.scan_folders), values[1]);
		}
