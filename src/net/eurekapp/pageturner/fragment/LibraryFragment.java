 ...
 */
package net.eurekapp.pageturner.fragment;
...
import net.eurekapp.pageturner.activity.CatalogActivity;
import net.eurekapp.pageturner.activity.FileBrowseActivity;
import net.eurekapp.pageturner.activity.LibraryActivity;
import net.eurekapp.pageturner.activity.PageTurnerActivity;
import net.eurekapp.pageturner.activity.ReadingActivity;
import net.eurekapp.pageturner.library.CleanFilesTask;
import net.eurekapp.pageturner.library.ImportCallback;
import net.eurekapp.pageturner.library.ImportTask;
import net.eurekapp.pageturner.library.KeyedQueryResult;
import net.eurekapp.pageturner.library.KeyedResultAdapter;
import net.eurekapp.pageturner.library.LibraryBook;
import net.eurekapp.pageturner.library.LibraryService;
import net.eurekapp.pageturner.library.QueryResult;
import net.eurekapp.pageturner.scheduling.QueueableAsyncTask;
import net.eurekapp.pageturner.scheduling.TaskQueue;
import net.eurekapp.pageturner.view.FastBitmapDrawable;
import net.eurekapp.ui.DialogFactory;
import net.eurekapp.ui.UiUtils;
import net.nightwhistler.htmlspanner.HtmlSpanner;
import net.eurekapp.pageturner.view.BookCaseView;
...
import static net.eurekapp.ui.UiUtils.onMenuPress;
...
public class LibraryFragment extends RoboSherlockFragment implements ImportCallback {
	...
	@InjectView(net.eurekapp.pageturner.R.id.libraryList)
	private ListView listView;
	
	@InjectView(net.eurekapp.pageturner.R.id.bookCaseView)
	private BookCaseView bookCaseView;
		
	@InjectView(net.eurekapp.pageturner.R.id.alphabetList)
	private ListView alphabetBar;
	
	private AlphabetAdapter alphabetAdapter;
	
	@InjectView(net.eurekapp.pageturner.R.id.alphabetDivider)
	private ImageView alphabetDivider;
	
	@InjectView(net.eurekapp.pageturner.R.id.libHolder)
	...
	public void onCreate(Bundle savedInstanceState) {			
		...
		Bitmap backupBitmap = BitmapFactory.decodeResource(getResources(), net.eurekapp.pageturner.R.drawable.unknown_cover1 );
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		return inflater.inflate(net.eurekapp.pageturner.R.layout.fragment_library, container, false);
	}
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		importDialog.setTitle(net.eurekapp.pageturner.R.string.importing_books);
		importDialog.setMessage(getString(net.eurekapp.pageturner.R.string.scanning_epub));
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<>(actionBar.getThemedContext(),
				...
				android.R.id.text1, getResources().getStringArray(net.eurekapp.pageturner.R.array.libraryQueries));
		...
		libraryFolder.match( folder -> {
			...
			Toast.makeText( context, net.eurekapp.pageturner.R.string.library_failed, Toast.LENGTH_LONG ).show();
		});
	...
	private void showBookDetails( final LibraryBook libraryBook ) {
		...
		builder.setTitle(net.eurekapp.pageturner.R.string.book_details);
		LayoutInflater inflater = PlatformUtil.getLayoutInflater(getActivity());
		
		View layout = inflater.inflate(net.eurekapp.pageturner.R.layout.book_details, null);
		builder.setView( layout );
		
		ImageView coverView = (ImageView) layout.findViewById(net.eurekapp.pageturner.R.id.coverImage );

		if ( libraryBook.getCoverImage() != null ) {
            Drawable coverDrawable = getCover(libraryBook).getOrElse(
                    getResources().getDrawable(net.eurekapp.pageturner.R.drawable.unknown_cover) );

            coverView.setImageDrawable(coverDrawable);
        }

		TextView titleView = (TextView) layout.findViewById(net.eurekapp.pageturner.R.id.titleField);
		TextView authorView = (TextView) layout.findViewById(net.eurekapp.pageturner.R.id.authorField);
		TextView lastRead = (TextView) layout.findViewById(net.eurekapp.pageturner.R.id.lastRead);
		TextView added = (TextView) layout.findViewById(net.eurekapp.pageturner.R.id.addedToLibrary);
		TextView descriptionView = (TextView) layout.findViewById(net.eurekapp.pageturner.R.id.bookDescription);
		TextView fileName = (TextView) layout.findViewById(net.eurekapp.pageturner.R.id.fileName);
		
		titleView.setText(libraryBook.getTitle());
		String authorText = String.format( getString(net.eurekapp.pageturner.R.string.book_by),
		...
		if (libraryBook.getLastRead() != null && ! libraryBook.getLastRead().equals(new Date(0))) {
			String lastReadText = String.format(getString(net.eurekapp.pageturner.R.string.last_read),
					DATE_FORMAT.format(libraryBook.getLastRead()));
			lastRead.setText( lastReadText );
		} else {
			String lastReadText = String.format(getString(net.eurekapp.pageturner.R.string.last_read), getString(net.eurekapp.pageturner.R.string.never_read));
			lastRead.setText( lastReadText );
		}

		String addedText = String.format( getString(net.eurekapp.pageturner.R.string.added_to_lib),
        ...
        builder.setNeutralButton(net.eurekapp.pageturner.R.string.delete, (dialog, which) -> {
		...
		builder.setPositiveButton(net.eurekapp.pageturner.R.string.read, (dialog, which) -> openBook(libraryBook) );
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {		
        inflater.inflate(net.eurekapp.pageturner.R.menu.library_menu, menu);
        ...
        UiUtils.onMenuPress(menu, net.eurekapp.pageturner.R.id.shelves_view).thenDo( toggleListener );
        UiUtils.onMenuPress(menu, net.eurekapp.pageturner.R.id.list_view).thenDo( toggleListener );

        UiUtils.onMenuPress(menu, net.eurekapp.pageturner.R.id.scan_books).thenDo( this::showImportDialog );
//        onMenuPress( menu, R.id.about ).thenDo( dialogFactory.buildAboutDialog()::show );

        UiUtils.onMenuPress(menu, net.eurekapp.pageturner.R.id.profile_day).thenDo(() -> switchToColourProfile(Configuration.ColourProfile.DAY) );
        UiUtils.onMenuPress(menu, net.eurekapp.pageturner.R.id.profile_night).thenDo(() -> switchToColourProfile(Configuration.ColourProfile.NIGHT));

        this.searchMenuItem = menu.findItem(net.eurekapp.pageturner.R.id.menu_search);
        ...
        if (searchMenuItem != null) {
            ...
            if (searchView != null) {
                ...
                searchMenuItem.setOnMenuItemClickListener( item -> {
                    dialogFactory.showSearchDialog(net.eurekapp.pageturner.R.string.search_library, net.eurekapp.pageturner.R.string.enter_query, this::performSearch);
                    return false;
                });
            }
        }
...
//        }
    ...
    private void launchFileManager() {
        ...
        try {
            ...
            Toast.makeText(getActivity(), getString(net.eurekapp.pageturner.R.string.install_oi),
	...
	public void onPrepareOptionsMenu(Menu menu) {
		...
		menu.findItem(net.eurekapp.pageturner.R.id.shelves_view).setVisible(! bookCaseActive);
		menu.findItem(net.eurekapp.pageturner.R.id.list_view).setVisible(bookCaseActive);
		menu.findItem(net.eurekapp.pageturner.R.id.profile_day).setVisible(config.getColourProfile() == Configuration.ColourProfile.NIGHT);
		menu.findItem(net.eurekapp.pageturner.R.id.profile_night).setVisible(config.getColourProfile() == Configuration.ColourProfile.DAY);
	}
	...
	private void showImportDialog() {
		...
		LayoutInflater inflater = PlatformUtil.getLayoutInflater(getActivity());
		final View layout = inflater.inflate(net.eurekapp.pageturner.R.layout.import_dialog, null);
		final RadioButton scanSpecific = (RadioButton) layout.findViewById(net.eurekapp.pageturner.R.id.radioScanFolder);
		final RadioGroup scanRadioGroup = (RadioGroup) layout.findViewById(net.eurekapp.pageturner.R.id.radioScanGroup);
		final TextView folder = (TextView) layout.findViewById(net.eurekapp.pageturner.R.id.folderToScan);
		final CheckBox copyToLibrary = (CheckBox) layout.findViewById(net.eurekapp.pageturner.R.id.copyToLib);
		final Button browseButton = (Button) layout.findViewById(net.eurekapp.pageturner.R.id.browseButton);
		...
		builder.setTitle(net.eurekapp.pageturner.R.string.import_books);
    ...
    private void afterImport(int booksImported, List<String> errors, boolean emptyLibrary, boolean silent,
        ...
        if ( (! errors.isEmpty()) ) {
            AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
            builder.setTitle(net.eurekapp.pageturner.R.string.import_errors);
        ...
        if ( booksImported > 0 ) {
            ...
            AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
            builder.setTitle(net.eurekapp.pageturner.R.string.no_books_found);

            if ( emptyLibrary ) {

                builder.setMessage( getString(net.eurekapp.pageturner.R.string.no_bks_fnd_text2) );
                ...
                builder.setMessage( getString(net.eurekapp.pageturner.R.string.no_new_books_found));
                builder.setNeutralButton(android.R.string.ok, ( dialog, which) -> dialog.dismiss() );
            }
	...
	public void importFailed(String reason, boolean silent) {
		...
		importDialog.hide();
		AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
		builder.setTitle(net.eurekapp.pageturner.R.string.import_failed);
	...
	private class BookListAdapter extends KeyedResultAdapter {	
		...
		public View getView(int index, final LibraryBook book, View convertView,
			...
			if ( convertView == null ) {			
				...
				rowView = inflater.inflate(net.eurekapp.pageturner.R.layout.book_row, parent, false);
			} else {
				rowView = convertView;
			}			
			
			TextView titleView = (TextView) rowView.findViewById(net.eurekapp.pageturner.R.id.bookTitle);
			TextView authorView = (TextView) rowView.findViewById(net.eurekapp.pageturner.R.id.bookAuthor);
			TextView dateView = (TextView) rowView.findViewById(net.eurekapp.pageturner.R.id.addedToLibrary);
			TextView progressView = (TextView) rowView.findViewById(net.eurekapp.pageturner.R.id.readingProgress);
			
			final ImageView imageView = (ImageView) rowView.findViewById(net.eurekapp.pageturner.R.id.bookCover);
			...
			String dateText = String.format(getString(net.eurekapp.pageturner.R.string.added_to_lib),
	...
	private class CoverScrollListener implements AbsListView.OnScrollListener {
        ...
        public  CoverScrollListener() {
            try {
                this.holoDrawable = getResources().getDrawable(net.eurekapp.pageturner.R.drawable.list_activated_holo);
	...
	private class BookCaseAdapter extends KeyedResultAdapter {
		...
		public View getView(final int index, final LibraryBook object, View convertView,
			...
			if ( convertView == null ) {				
				LayoutInflater inflater = PlatformUtil.getLayoutInflater(getActivity());
				result = inflater.inflate(net.eurekapp.pageturner.R.layout.bookcase_row, parent, false);
			...
			final ImageView image = (ImageView) result.findViewById(net.eurekapp.pageturner.R.id.bookCover);
			image.setImageDrawable(backupCover);
			TextView text = (TextView) result.findViewById(net.eurekapp.pageturner.R.id.bookLabel);
			text.setText( object.getTitle() );
			text.setBackgroundResource(net.eurekapp.pageturner.R.drawable.alphabet_bar_bg_dark);
	...
	private void buildImportQuestionDialog() {
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
		builder.setTitle(net.eurekapp.pageturner.R.string.no_books_found);
		builder.setMessage( getString(net.eurekapp.pageturner.R.string.scan_bks_question) );
	...
	private class AlphabetAdapter extends ArrayAdapter<Character> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if ( tag.equals(highlightChar) ) {
				view.setBackgroundDrawable( getResources().getDrawable(net.eurekapp.pageturner.R.drawable.list_activated_holo));
			} else {
    ...
    private void loadQueryData( QueryResult<LibraryBook> result ) {
        ...
        if ( result instanceof KeyedQueryResult && result.getSize() >= ALPHABET_THRESHOLD ) {
            ...
            alphabetAdapter = new AlphabetAdapter(getActivity(),
                    net.eurekapp.pageturner.R.layout.alphabet_line, net.eurekapp.pageturner.R.id.alphabetLabel,	keyedResult.getAlphabet() );
	...
	private class LoadBooksTask extends QueueableAsyncTask<String, Integer, QueryResult<LibraryBook>> {
        ...
        public void doOnPostExecute(Option<QueryResult<LibraryBook>> result) {
            ...
            }, () -> Toast.makeText(context, net.eurekapp.pageturner.R.string.library_failed, Toast.LENGTH_SHORT).show());
